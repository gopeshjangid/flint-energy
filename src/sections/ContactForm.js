...
const leadSubmitHandler = async () => {
    setMessage({...message , message : messages.FORM_SUBMITING , type  : "info" ,open : true});
    try {
        const obj = {
            firstName : firstName,
            lastName : lastName,
        ...
        if(res["all_ok"]) {
            setMessage({...message , message : messages.OTP_SENT , severity  : "success" ,open : true});
            setIsOtpSent(true); 
        } else {
          setMessage({...message , message : res["error_msg"] , severity  : "error" ,open : true});
        }
    } catch (err) {
...
const verifyOtpHandler = async () => {
   setMessage({message : "Verifying..." , severity  : "info" ,open : true});
    try {
        ...
        if(res["all_ok"]) {
            setMessage({ message : messages.OTP_VERIFIED , severity  : "success" ,open : true});
  ...
  return (
    <Box id="verification" as="section" sx={styles.section}>
      ...
      <Container>
        <SectionHeading
          slogan="Verification"
          title="Verify with your contact details."
        />
        ...
        <form className={classes.form} noValidate>
          <Grid container spacing={2}>
            ...
            <>
              ...
              <Grid item xs={12}>
              <Label mb={3}>
                <Checkbox required color="secondary" onChange={()=> setCheckbox (!checked)} checked={checked} name="saveAddress" value="yes" />
                <Text sx={styles.checkboxLabel}>have read Flint Energy Privacy Policy</Text> 
              </Label>
        </Grid>
            </>
            }
            {
              ...
              <Grid item xs={12}>
                <TextField
                  ...
                  id="otp"
                />&nbsp;&nbsp;
                <Link onClick={leadSubmitHandler}>
                  Resend OTP
                </Link>
            </Grid>
           }
          ...
          <Button
            ...
            variant="contained"
            color="primary"
            onClick={ !isOtpSent ? () => {leadSubmitHandler();} : verifyOtpHandler}
